...
export default function PublicMonitor() {
  ...
  const filteredBarang = useMemo(() => {
    ...
    return sorted.filter((b) => {
      const matchSearch = !barangSearch || [b.uidbarang, b.namabarang, b.kategori, b.lastupdate].some((v) => String(v || "").toLowerCase().includes(barangSearch.toLowerCase()));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-8">
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="search" placeholder="Cari UID barang, nama barang, kategori, update..." value={barangSearch} onChange={(e) => setBarangSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" /></div>
