...
const EMPTY: Barang = { uidbarang: "", namabarang: "", kategori: "", dipinjam: "false", lastuser: "", lastkelas: "", lastupdate: "", lastuid: "" };
...
export default function BarangPage() {
  ...
  const filtered = useMemo(() => {
    const base = deadlineSort === "none"
      ? (sortMode === "newest_first" ? [...barang].reverse() : [...barang])
      : sortItemsByDeadline(barang, riwayat, perpanjang, deadlineSort);
  ...
  }, [barang, riwayat, perpanjang, search, filterKategori, filterStatus, sortMode, deadlineSort]);
    ...
    <div className="page-transition space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <SyncButton />
          <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-sm font-semibold rounded-lg hover:opacity-90 transition-opacity shadow-sm"><Plus className="w-4 h-4" />Tambah Barang</button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <select value={filterKategori} onChange={e => setFilterKategori(e.target.value)} className="py-2 pl-3 pr-8 text-sm bg-card border border-border rounded-lg focus:outline-none focus:border-primary/50"><option value="">Semua Kategori</option>{kategoriList.map(k => <option key={k} value={k}>{k}</option>)}</select>
          <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="py-2 pl-3 pr-8 text-sm bg-card border border-border rounded-lg focus:outline-none focus:border-primary/50"><option value="">Semua Status</option><option value="tersedia">Tersedia</option><option value="dipinjam">Dipinjam</option></select>
          <select value={deadlineSort} onChange={e => setDeadlineSort(e.target.value as DeadlineSort)} className="py-2 pl-3 pr-8 text-sm bg-card border border-border rounded-lg focus:outline-none focus:border-primary/50"><option value="none">Sort Tenggat: Default</option><option value="asc">Tenggat Terdekat</option><option value="desc">Tenggat Terjauh</option></select>
      ...
      <div className="bg-card border border-card-border rounded-xl shadow-xs overflow-hidden">
        {isLoading && !barang.length ? <div className="flex items-center justify-center py-16"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div> : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground"><Package className="w-10 h-10 mb-3 opacity-30" /><p className="text-sm font-medium">Tidak ada data barang</p><p className="text-xs mt-1">{search || filterKategori || filterStatus ? "Ubah filter" : "Tambah barang untuk memulai"}</p></div>
        ) : (
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-muted/50">{["UID Barang","Nama Barang","Kategori","Status","Last User","Last Kelas","Max Kembali","Last UID","Aksi"].map(h => <th key={h} className="text-left px-4 py-3 font-semibold text-muted-foreground text-xs uppercase tracking-wider whitespace-nowrap">{h}</th>)}</tr></thead><tbody className="divide-y divide-border">{rows.map((b, i) => { const localId = (b as any)._localId as string | undefined; const red = isItemOverdue(b, riwayat, perpanjang); return <tr key={b.uidbarang || i} className={`${red ? "barang-belum-kembali" : "hover:bg-muted/30"} transition-colors`}><td className="px-4 py-3 font-mono text-xs text-muted-foreground whitespace-nowrap"><div className="flex items-center gap-2">{b.uidbarang || "-"}<SyncBadge localId={localId} pendingIds={pendingIds} failedIds={failedIds} /></div></td><td className="px-4 py-3 font-medium text-foreground whitespace-nowrap">{b.namabarang || "-"}</td><td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{b.kategori || "-"}</td><td className="px-4 py-3"><StatusBadge value={b.dipinjam} /></td><td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{b.lastuser || "-"}</td><td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{b.lastkelas || "-"}</td><td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{maxKembali(b)}</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground whitespace-nowrap">{b.lastuid || "-"}</td><td className="px-4 py-3"><button onClick={() => openEdit(b)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-primary/10 text-primary rounded-lg hover:bg-primary/20 transition-colors whitespace-nowrap"><Edit2 className="w-3 h-3" />Edit</button></td></tr>; })}</tbody></table></div>
        )}
      </div>
