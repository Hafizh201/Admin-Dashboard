...
export default function PublicMonitor() {
  ...
  const filteredBarang = useMemo(() => {
    ...
    return sorted.filter((b) => {
      const matchSearch = !barangSearch || [b.uidbarang, b.namabarang, b.kategori, b.lastuser, b.lastkelas].some((v) => String(v || "").toLowerCase().includes(barangSearch.toLowerCase()));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Halaman publik hanya menampilkan status barang dan riwayat peminjaman. Fitur admin disembunyikan.</p>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-3">
            <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><input type="search" placeholder="Cari UID barang, nama barang, kategori, last user, kelas..." value={barangSearch} onChange={(e) => setBarangSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100" /></div>
